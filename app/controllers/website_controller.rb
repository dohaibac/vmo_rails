class WebsiteController < ApplicationController
  ...
  def donate
    ...
    if charity && params[:omise_token].present? && !(params[:amount].blank? || params[:amount].to_i <= 20)
      if Rails.env.test?
    ...
    elsif (params[:amount].blank? || params[:amount].to_i <= 20)
      donate_fail(retrieve_token(params[:omise_token]))
    else
      donate_fail()
    end
